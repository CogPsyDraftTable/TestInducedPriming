...
for(i=5;i<=9;i++){
	for(j=1;j<=15;j++){
	acount++
	bowlA[acount]=[StimList[i][j],StimList[i][0],"Related","Pre-Lure",j,"New"] 
	}
}

for(i=10;i<=14;i++){
	acount++
	bowlA[acount]=[StimList[i][0],StimList[i][0],"UPCategory","Pre-Lure",0,"New"] //this is the group we hope to create false positives with 
}


//make bowl B
//add OLD related words
bowlB=[[]];
bcount=-1
for(i=0;i<=4;i++){
	for(j=1;j<=7;j++){
	bcount++
	bowlB[bcount]=[StimList[i][j],StimList[i][0],"ORelated","Post-Lure",j,"Old"]
	}
}

//add NEW related words
for(i=15;i<=19;i++){
	for(j=1;j<=6;j++){
	bcount++
	bowlB[bcount]=[StimList[i][j],StimList[i][0],"NRelated","Post-Lure",j,"New"]
	}
}

//add studied DRM category words
for(i=0;i<=4;i++){
	bcount++
	bowlB[bcount]=[StimList[i][0],StimList[i][0],"SCategory","Post-Lure",0,"New"] //this is the group we hope to create false positives with 
}

//add post-lure nonstudied category words
for(i=5;i<=9;i++){
	bcount++
	bowlB[bcount]=[StimList[i][0],StimList[i][0],"NSCategory","Post-Lure",0,"New"] //this is the group we hope to create false positives with 
}

//add repeated unprimed category words
for(i=10;i<=14;i++){
	bcount++
	bowlB[bcount]=[StimList[i][0],StimList[i][0],"UPCategory","Post-Lure",0,"New"] //this is the group we hope to create false positives with 
}
